...
<form id="challengeForm">
  ...
  <label>最大高度<input id="height" type="number" min="1" max="64" step="1" value="7" required></label>
...
onFrame = currentTime => {
  ...
  $("beatHint").textContent = `${progress} · ${side} · ` + (value === 0 ? "空拍" : value === 2 ? "持球" : `${value} 拍後由${(t + value) % 2 === 0 ? "左手" : "右手"}接住`);
};
async function nextChallenge() {
  if ($("generate").disabled) return;
  $("generate").disabled = true;
  try {
    const settings = Object.fromEntries(["balls", "height", "minimum", "maximum"].map(id => [id, $(id).value === "" ? NaN : Number($(id).value)]));
