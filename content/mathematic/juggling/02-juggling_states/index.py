...
def _(mo):
    mo.md(r"""
    輸入格式與上一篇的動畫相同：`3_2` 表示三拍後到手 $2$，
    同一手的 multiplex 寫成 `[3_2,3_2]`，單顆球可省略中括號，空手寫 `0` 或 `[]`。
    同一拍以 `|` 分隔各手，換行或空白接下一拍。
    單手可省略 `_1`，例如 `9 7 5 3 1` 是五拍，`[3,4]` 表示同時拋兩顆球。
    時長使用十進位整數或 siteswap 的單字母：`a_2` 與 `10_2` 都是 $10$ 拍，
    `p_2` 與 `25_2` 都是 $25$ 拍，接球手都是 $2$。
    `p_2` 中的 `p` 是時長，底線 `_` 才是分隔符。
    連續數字一律代表一個整數：`25` 與 `[25]` 都只有一顆、時長 $25$ 的球，
    `[34]` 也只有一顆、時長 $34$ 的球；不同拍必須分隔，multiplex 內必須用逗號。
    因此 `97531` 不再拆成五拍，而會因時長超過上限被拒絕。
    舊的大括號與 tuple 格式也採用相同的十進位讀法。
    舊的 `3p2`、`pp2` 不再接受，請改用 `3_2`、`p_2`；單獨的 `p` 仍表示 $25$ 拍。
    圖上的投擲標籤也使用這套記號。
    """)
    return
...
def _(mo, set_beat):
    pattern_input = mo.ui.text(
        value=(
            "[1_1,2_2,3_3]|2_3|0 1_3|0|0 "
            "0|3_2|[2_3,3_3] 0|0|3_2 "
            "0|0|2_1 0|1_1|1_1"
        ),
        label="pattern",
        full_width=True,
